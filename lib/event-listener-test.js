...
class EventListenerTest {
	...
	constructor(handler, rules, { before, after, mustHaveClient, mustHaveId }) {

		this.handler = handler;
		this.rules = rules;

		this.mustHaveClient = !!mustHaveClient;
		this.mustHaveId = !!mustHaveId;
	...
	process() {
		...
		this.addGettersRules(this.mustHaveClient, this.mustHaveId);

		this.rules.forEach(rule => this.processRule(rule));
	}

	addGettersRules(mustHaveClient, mustHaveId) {
		// @todo Agregar rules hardcodeadas para la validación de los getters. Quizás stubeando cosas para que solo ejecute la validación.
		// Ver cómo chota hacerlo
	}
	...
	processRule({
		...
		it(description, async () => {
			...
			try {
				...
				if(e.name !== 'Error') // It may be an error from sls-api-response. Everything will be checked below.
			...
			if(after)
				await after(sinon, ApiResponse.send);
		});
	}
}
